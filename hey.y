%{
	...
	vector< map<string,string> > symbol_table(1000);
...
variable_declaration	:	type_specifier identifier  SEMI
				{
					...
					{
						semanticError = true;
						cout << "Multiple declaration of " << ($2.node_holder)->id_node << " at line no. " << mylineno+1 << endl;
			...
			|	incr identifier SEMI
				{
					...
					{
						semanticError = true;
						cout << "Undeclared variable " << ($2.node_holder)->id_node << " line no. " << mylineno+1 << endl;	
					}
					else
					{
						($$.node_holder)->dtype_node = symbol_table[scope_2][($1.node_holder)->id_node];
						...
						| number operator expression
									{
										...
										if($2.s_holder == NULL)
										{
											new_node_struct->code_node = "" ;
										}
										else
										{
											new_node_struct->code_node = string($2.s_holder);
										}
										(new_node_struct->v_node).push_back($1.node_holder);
										(new_node_struct->v_node).push_back($3.node_holder);
...
string code_generation(node_struct *n)
{
	...
	if(n->code_node=="Function_Statements2")
	{
		...
		code_generation((n->v_node)[1]);
		cout<<"call"<<" "<<code_generation((n->v_node)[0])<<" "<<counter_par<<endl;
		counter_par=0;
		return "";
